...
from foro.managers.topic import TopicManager
from foro.utils.models import AutoSlugField
...
class Topic(models.Model):
    ...
    def get_absolute_url(self):
        if self.category_id == settings.ST_TOPIC_PRIVATE_CATEGORY_PK:
            return reverse('foro:private-detail', kwargs={'topic_id': str(self.id), 'slug': self.slug})

        return reverse('foro:topic-detail', kwargs={'pk': str(self.id), 'slug': self.slug})
