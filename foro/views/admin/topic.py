...
def topic_deleted(request):
    ...
    return render(request, 'foro/admin/topic/topic_deleted.html', {'topics': topics, })


@administrator_required
def topic_closed(request):
    topics = Topic.objects.filter(is_closed=True)
    return render(request, 'foro/admin/topic/topic_closed.html', {'topics': topics, })


@administrator_required
def topic_pinned(request):
    topics = Topic.objects.filter(is_pinned=True)
    return render(request, 'foro/admin/topic/topic_pinned.html', {'topics': topics, })
