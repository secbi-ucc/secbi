...
from foro.forms.admin import UserEditForm
...
def user_edit(request, user_id):
    ...
    return render(request, 'foro/admin/user/user_edit.html', {'form': form, })


@administrator_required
def user_list(request):
    users = User.objects.all()
    return render(request, 'foro/admin/user/user_list.html', {'users': users, })


@administrator_required
def user_admins(request):
    users = User.objects.filter(is_administrator=True)
    return render(request, 'foro/admin/user/user_admins.html', {'users': users, })


@administrator_required
def user_mods(request):
    users = User.objects.filter(is_moderator=True, is_administrator=False)
    return render(request, 'foro/admin/user/user_mods.html', {'users': users, })


@administrator_required
def user_unactive(request):
    users = User.objects.filter(is_active=False)
    return render(request, 'foro/admin/user/user_unactive.html', {'users': users, })
