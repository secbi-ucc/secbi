...
from foro.models.topic_notification import TopicNotification, comment_posted, \
    COMMENT, MENTION, topic_private_post_create, topic_private_access_pre_create, topic_viewed
from foro.forms.topic_notification import NotificationCreationForm, NotificationForm
from foro.templatetags.tags.topic_notification import render_notification_form, has_topic_notifications
...
class TopicNotificationViewTest(TestCase):
    ...
    def test_topic_notification_list(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse('foro:topic-notification-list'))
    ...
    def test_topic_notification_list_show_private_topic(self):
        ...
        response = self.client.get(reverse('foro:topic-notification-list-unread'))
        self.assertQuerysetEqual(response.context['page'], map(repr, [topic_notif, ]))

        # ajax list should behave the same
        response = self.client.get(reverse('foro:topic-notification-ajax'),
    ...
    def test_topic_notification_list_unread(self):
        ...
        # fake next page
        response = self.client.get(reverse('foro:topic-notification-list-unread') + "?notif=" + str(topic_notification.pk))
    ...
    def test_topic_notification_create(self):
        ...
        response = self.client.post(reverse('foro:topic-notification-create', kwargs={'topic_id': self.topic.pk, }),
                                    form_data)
    ...
    def test_topic_notification_create_has_access(self):
        ...
        utils.login(self)
        form_data = {'is_active': True, }
        response = self.client.post(reverse('foro:topic-notification-create', kwargs={'topic_id': private.topic.pk, }),
                                    form_data)
    ...
    def test_topic_notification_update(self):
        ...
        utils.login(self)
        form_data = {'is_active': True, }
        response = self.client.post(reverse('foro:topic-notification-update',
    ...
    def test_topic_notification_update_invalid_user(self):
        ...
        utils.login(self)
        form_data = {}
        response = self.client.post(reverse('foro:topic-notification-update', kwargs={'pk': notification.pk, }),
                                    form_data)
        self.assertEqual(response.status_code, 404)
