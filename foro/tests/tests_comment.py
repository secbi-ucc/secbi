...
from foro.models.comment import Comment,\
    ...
    topic_post_moderate
from foro.forms.comment import CommentForm, CommentMoveForm, CommentImageForm
from foro.signals.comment import comment_post_update, comment_posted, comment_pre_update, comment_moved
from foro.templatetags.tags.comment import render_comments_form
from foro.utils import markdown
from foro.views.comment import comment_delete
...
class CommentViewTest(TestCase):
    ...
    def test_comment_publish(self):
        ...
        response = self.client.post(reverse('foro:comment-publish', kwargs={'topic_id': self.topic.pk, }),
                                    form_data)
        expected_url = reverse('foro:comment-find', kwargs={'pk': 1, })
        ...
        # get
        response = self.client.get(reverse('foro:comment-publish', kwargs={'topic_id': self.topic.pk, }))
    ...
    def test_comment_publish_on_private(self):
        ...
        utils.login(self)
        form_data = {'comment': 'foobar', }
        response = self.client.post(reverse('foro:comment-publish', kwargs={'topic_id': private.topic.pk, }),
                                    form_data)
        expected_url = reverse('foro:comment-find', kwargs={'pk': 1, })
        self.assertRedirects(response, expected_url, status_code=302, target_status_code=302)
        self.assertEqual(len(Comment.objects.all()), 1)
    ...
    def test_comment_publish_on_removed_topic_or_category(self):
        ...
        utils.login(self)
        form_data = {'comment': 'foobar', }
        response = self.client.post(reverse('foro:comment-publish', kwargs={'topic_id': topic2.pk, }),
                                    form_data)
        self.assertEqual(response.status_code, 404)
    ...
    def test_comment_update(self):
        ...
        response = self.client.post(reverse('foro:comment-update', kwargs={'pk': comment.pk, }),
                                    form_data)
        expected_url = reverse('foro:comment-find', kwargs={'pk': 1, })
        self.assertRedirects(response, expected_url, status_code=302, target_status_code=302)
    ...
    def test_comment_update_signal(self):
        ...
        form_data = {'comment': 'barfoo', }
        response = self.client.post(reverse('foro:comment-update', kwargs={'pk': comment_posted.pk, }),
                                    form_data)
    ...
    def test_comment_delete(self):
        ...
        utils.login(self)
        form_data = {}
        response = self.client.post(reverse('foro:comment-delete', kwargs={'pk': comment.pk, }),
                                    form_data)
        expected_url = comment.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302, target_status_code=302)

        response = self.client.get(reverse('foro:comment-delete', kwargs={'pk': comment.pk, }))
        self.assertEqual(response.status_code, 200)
    ...
    def test_comment_undelete(self):
        ...
        utils.login(self)
        form_data = {}
        response = self.client.post(reverse('foro:comment-undelete', kwargs={'pk': comment.pk, }),
                                    form_data)
        expected_url = comment.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302, target_status_code=302)

        response = self.client.get(reverse('foro:comment-undelete', kwargs={'pk': comment.pk, }))
        self.assertEqual(response.status_code, 200)
    ...
    def test_comment_move(self):
        ...
        response = self.client.post(reverse('foro:comment-move', kwargs={'topic_id': self.topic.pk, }),
                                    form_data)
    ...
    def test_comment_find(self):
        ...
        """
        comment = utils.create_comment(user=self.user, topic=self.topic)
        response = self.client.post(reverse('foro:comment-find', kwargs={'pk': comment.pk, }))
    ...
    def test_comment_image_upload(self):
        ...
        self.assertEqual(res['url'], os.path.join(settings.MEDIA_URL, 'foro', 'images', str(self.user.pk),
                                                  "bf21c3043d749d5598366c26e7e4ab44.gif").replace("\\", "/"))
        os.remove(os.path.join(settings.MEDIA_ROOT, 'foro', 'images', str(self.user.pk),
...
class CommentFormTest(TestCase):
    ...
    def test_comment_markdown(self):
        form_data = {'comment': '**foro unicode: áéíóú** '
                                '<script>alert();</script>', }
        form = CommentForm(data=form_data)
        self.assertEqual(form.is_valid(), True)
        form.user = self.user
        ...
        comment = form.save()
        self.assertEqual(comment.comment_html, '<p><strong>foro unicode: áéíóú</strong> '
    ...
    def test_comment_image_upload(self):
        ...
        image_url = os.path.join(settings.MEDIA_URL, 'foro', 'images', str(self.user.pk),
                                 image.name).replace("\\", "/")
        self.assertEqual(image.url, image_url)
        image_path = os.path.join(settings.MEDIA_ROOT, 'foro', 'images', str(self.user.pk), image.name)
    ...
    def test_comment_image_upload_no_extension(self):
        ...
        image = form.save()
        self.assertEqual(image.name, "bf21c3043d749d5598366c26e7e4ab44.gif")
        os.remove(os.path.join(settings.MEDIA_ROOT, 'foro', 'images', str(self.user.pk), image.name))
