...
from foro.forms.user import RegistrationForm, UserProfileForm, EmailChangeForm, ResendActivationForm
from foro.backends.user import EmailAuthBackend
...
from foro.models.user import User as UserModel
...
class UserViewTest(TestCase):
    ...
    def test_login_email(self):
        ...
        """
        # get
        response = self.client.get(reverse('foro:user-login'))
        self.assertEqual(response.status_code, 200)

        # post
        form_data = {'username': self.user.email, 'password': "bar"}
        response = self.client.post(reverse('foro:user-login'),
                                    form_data)
    ...
    def test_login_redirect(self):
        ...
        self.assertRedirects(response, expected_url, status_code=302)
        # next
        response = self.client.get(reverse('foro:user-login') + '?next=/fakepath/')
    ...
    def test_register(self):
        ...
        """
        # get
        response = self.client.get(reverse('foro:user-register'))
        self.assertEqual(response.status_code, 200)

        # post
        form_data = {'username': 'uniquefoo', 'email': 'some@some.com', 'password1': 'pass', 'password2': 'pass'}
        response = self.client.post(reverse('foro:user-register'),
                                    form_data)
        expected_url = reverse('foro:user-login')
        self.assertRedirects(response, expected_url, status_code=302)

        # redirect logged in user
        utils.login(self)
        response = self.client.get(reverse('foro:user-register'))
        self.assertRedirects(response, reverse('foro:profile-update'), status_code=302)
    ...
    def test_register_email_sent(self):
        ...
        """
        form_data = {'username': 'uniquefoo', 'email': 'some@some.com', 'password1': 'pass', 'password2': 'pass'}
        response = self.client.post(reverse('foro:user-register'), form_data)
    ...
    def test_register_next_logged_in(self):
        ...
        # redirect logged in user
        utils.login(self)
        response = self.client.get(reverse('foro:user-register') + "?next=/fakepath/")
        self.assertRedirects(response, '/fakepath/', status_code=302, target_status_code=404)
    ...
    def test_profile_topics(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse("foro:profile-topics", kwargs={'pk': self.user2.pk,
    ...
    def test_profile_topics_invalid_slug(self):
        ...
        response = self.client.get(reverse("foro:profile-topics", kwargs={'pk': self.user2.pk,
                                                                            'slug': "invalid"}))
        expected_url = reverse("foro:profile-topics", kwargs={'pk': self.user2.pk,
                                                                'slug': self.user2.slug})
    ...
    def test_profile_comments(self):
        ...
        response = self.client.get(reverse("foro:profile-detail", kwargs={'pk': self.user2.pk,
                                                                            'slug': self.user2.slug}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_profile_comments_invalid_slug(self):
        ...
        response = self.client.get(reverse("foro:profile-detail", kwargs={'pk': self.user2.pk,
                                                                            'slug': "invalid"}))
        expected_url = reverse("foro:profile-detail", kwargs={'pk': self.user2.pk,
                                                                'slug': self.user2.slug})
        self.assertRedirects(response, expected_url, status_code=301)
    ...
    def test_profile_likes(self):
        ...
        response = self.client.get(reverse("foro:profile-likes", kwargs={'pk': self.user2.pk,
                                                                           'slug': self.user2.slug}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_profile_likes_invalid_slug(self):
        ...
        response = self.client.get(reverse("foro:profile-likes", kwargs={'pk': self.user2.pk,
                                                                           'slug': "invalid"}))
        expected_url = reverse("foro:profile-likes", kwargs={'pk': self.user2.pk,
                                                               'slug': self.user2.slug})
        self.assertRedirects(response, expected_url, status_code=301)
    ...
    def test_profile_update(self):
        ...
        utils.login(self)
        # get
        response = self.client.get(reverse('foro:profile-update'))
        self.assertEqual(response.status_code, 200)

        # post
        form_data = {'first_name': 'foo', 'last_name': 'bar',
                     'location': 'foro', 'timezone': self.user.timezone}
        response = self.client.post(reverse('foro:profile-update'),
                                    form_data)
        expected_url = reverse('foro:profile-update')
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_login_rate_limit(self):
        ...
        url = reverse('foro:user-login') + "?next=/path/"
        for _ in range(6):
    ...
    def test_custom_reset_password(self):
        ...
        for _ in range(6):
            response = self.client.post(reverse('foro:password-reset'),
                                        form_data)
        expected_url = reverse("foro:password-reset")
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_password_reset_confirm(self):
        ...
        test access
        """
        response = self.client.get(reverse('foro:password-reset-confirm', kwargs={'uidb64': 'f-a-k-e',
                                                                                    'token': 'f-a-k-e'}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_admin_login(self):
        ...
        expected_url = reverse("foro:user-login") + "?next=" + reverse('admin:login')
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_profile_password_change(self):
        ...
        form_data = {'old_password': 'foo',
                     'new_password1': 'bar',
                     'new_password2': 'bar'}
        response = self.client.post(reverse('foro:profile-password-change'),
                                    form_data)
        expected_url = reverse("foro:profile-update")
        self.assertRedirects(response, expected_url, status_code=302)
        ...
        # get
        response = self.client.get(reverse('foro:profile-password-change'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_registration_activation(self):
        ...
        response = self.client.get(reverse('foro:registration-activation', kwargs={'pk': self.user.pk,
                                                                                     'token': token}))
        expected_url = reverse("foro:user-login")
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_email_change_confirm(self):
        ...
        response = self.client.get(reverse('foro:email-change-confirm', kwargs={'token': token}))
        expected_url = reverse("foro:profile-update")
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_profile_email_change(self):
        ...
        response = self.client.post(reverse('foro:profile-email-change'),
                                    form_data)
        expected_url = reverse("foro:profile-update")
        self.assertRedirects(response, expected_url, status_code=302)
        self.assertEquals(len(mail.outbox), 1)
        ...
        # get
        response = self.client.get(reverse('foro:profile-email-change'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_resend_activation_email(self):
        ...
        form_data = {'email': user.email,
                     'password': "foo"}
        response = self.client.post(reverse('foro:resend-activation'),
                                    form_data)
        expected_url = reverse("foro:user-login")
        self.assertRedirects(response, expected_url, status_code=302)
        self.assertEquals(len(mail.outbox), 1)
        ...
        # get
        response = self.client.get(reverse('foro:resend-activation'))
    ...
    def test_logout(self):
        ...
        response = self.client.get(reverse('foro:user-logout'))
        self.assertEqual(response.status_code, 200)
        self.assertTrue(self.client.session.items())

        # post should log out the user (clear the session)
        response = self.client.post(reverse('foro:user-logout'))
        expected_url = "/"
        self.assertRedirects(response, expected_url, status_code=302)
        ...
        utils.login(self)
        self.assertTrue(self.client.session.items())
        response = self.client.post(reverse('foro:user-logout') + '?next=/fakepath/')
        self.assertRedirects(response, '/fakepath/', status_code=302, target_status_code=404)
        self.assertFalse(self.client.session.items())
    ...
    def test_logout_anonymous_redirect(self):
        ...
        response = self.client.get(reverse('foro:user-logout') + '?next=/fakepath/')
        self.assertRedirects(response, '/fakepath/', status_code=302, target_status_code=404)
