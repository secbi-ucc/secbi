...
from foro.models.topic_poll import TopicPoll, TopicPollChoice, TopicPollVote
from foro.forms.topic_poll import TopicPollForm, TopicPollChoiceFormSet, TopicPollVoteManyForm
from foro.signals.topic_poll import topic_poll_post_vote, topic_poll_pre_vote
from foro.templatetags.tags.topic_poll import render_poll_form
...
class TopicPollViewTest(TestCase):
    ...
    def test_poll_update_logged_in(self):
        ...
        """
        response = self.client.get(reverse('foro:poll-update', kwargs={'pk': 1, }))
    ...
    def test_poll_update_wrong_user(self):
        ...
        response = self.client.get(reverse('foro:poll-update', kwargs={'pk': poll.pk, }))
    ...
    def test_poll_update_post(self):
        ...
        form_data = {'choices-TOTAL_FORMS': 2, 'choices-INITIAL_FORMS': 0,
                     ...
                     'choice_limit': 2}
        response = self.client.post(reverse('foro:poll-update', kwargs={'pk': poll.pk, }),
                                    form_data)
    ...
    def test_poll_close_logged_in(self):
        ...
        """
        poll = TopicPoll.objects.create(topic=self.topic)
        response = self.client.get(reverse('foro:poll-close', kwargs={'pk': poll.pk, }))
        self.assertEqual(response.status_code, 302)
    ...
    def test_poll_close_post(self):
        ...
        poll = TopicPoll.objects.create(topic=self.topic)
        form_data = {}
        response = self.client.post(reverse('foro:poll-close', kwargs={'pk': poll.pk, }),
                                    form_data)
        expected_url = poll.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_poll_vote_logged_in(self):
        ...
        response = self.client.post(reverse('foro:poll-vote', kwargs={'pk': poll.pk, }),
                                    form_data)
        expected_url = reverse('foro:user-login') + "?next=" + poll.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_poll_vote_get(self):
        ...
        utils.login(self)
        poll = TopicPoll.objects.create(topic=self.topic)
        response = self.client.get(reverse('foro:poll-vote', kwargs={'pk': poll.pk, }))
