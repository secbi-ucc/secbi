...
from foro.forms.search import BasicSearchForm, AdvancedSearchForm
from foro.templatetags.tags.search import render_search_form
from foro.search_indexes import TopicIndex
...
class SearchViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.topic = utils.create_topic(category=self.category, user=self.user, title="foro search test")
    ...
    def test_advanced_search_detail(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse('foro:search'))
    ...
    def test_advanced_search_in_category(self):
        ...
        utils.login(self)
        category = utils.create_category()
        data = {'q': 'foro search', 'category': category.pk}
        response = self.client.get(reverse('foro:search'),
                                   data)
