...
from foro.models.comment import MOVED, CLOSED, UNCLOSED, PINNED, UNPINNED
from foro.models.topic import Topic, topic_viewed, comment_posted, comment_moved
from foro.forms.topic import TopicForm
from foro.signals.topic import topic_post_moderate
...
class TopicViewTest(TestCase):
    ...
    def test_topic_publish(self):
        ...
        response = self.client.post(reverse('foro:topic-publish'),
                                    form_data)
        ...
        # get
        response = self.client.get(reverse('foro:topic-publish'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_topic_publish_in_category(self):
        ...
        form_data = {'comment': 'foo', 'title': 'foobar', 'category': category.pk,
                     'choices-TOTAL_FORMS': 2, 'choices-INITIAL_FORMS': 0, 'choice_limit': 1}
        response = self.client.post(reverse('foro:topic-publish', kwargs={'category_id': category.pk, }),
                                    form_data)
        topic = Topic.objects.last()
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_topic_publish_in_subcategory(self):
        ...
        form_data = {'comment': 'foo', 'title': 'foobar', 'category': subcategory.pk,
                     'choices-TOTAL_FORMS': 2, 'choices-INITIAL_FORMS': 0, 'choice_limit': 1}
        response = self.client.post(reverse('foro:topic-publish', kwargs={'category_id': subcategory.pk, }),
                                    form_data)
        topic = Topic.objects.last()
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_topic_publish_invalid_category(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse('foro:topic-publish', kwargs={'category_id': str(99), }))
    ...
    def test_topic_update(self):
        ...
        response = self.client.post(reverse('foro:topic-update', kwargs={'pk': topic.pk, }),
                                    form_data)
    ...
    def test_topic_detail_view(self):
        ...
        category = utils.create_category()
        topic = utils.create_topic(category=category)
        response = self.client.get(reverse('foro:topic-detail', kwargs={'pk': topic.pk, 'slug': topic.slug}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_topic_detail_view_invalid_slug(self):
        ...
        category = utils.create_category()
        topic = utils.create_topic(category=category)
        response = self.client.get(reverse('foro:topic-detail', kwargs={'pk': topic.pk,
                                                                          'slug': 'bar'}))
    ...
    def test_topic_active_view(self):
        ...
        response = self.client.get(reverse('foro:topic-active'))
    ...
    def test_topic_moderate_delete(self):
        ...
        form_data = {}
        response = self.client.post(reverse('foro:topic-delete', kwargs={'pk': topic.pk, }),
                                    form_data)
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_topic_moderate_undelete(self):
        ...
        form_data = {}
        response = self.client.post(reverse('foro:topic-undelete', kwargs={'pk': topic.pk, }),
                                    form_data)
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_topic_moderate_lock(self):
        ...
        topic = utils.create_topic(category)
        form_data = {}
        response = self.client.post(reverse('foro:topic-lock', kwargs={'pk': topic.pk, }),
                                    form_data)
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_topic_moderate_unlock(self):
        ...
        form_data = {}
        response = self.client.post(reverse('foro:topic-unlock', kwargs={'pk': topic.pk, }),
                                    form_data)
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_topic_moderate_pin(self):
        ...
        topic = utils.create_topic(category)
        form_data = {}
        response = self.client.post(reverse('foro:topic-pin', kwargs={'pk': topic.pk, }),
                                    form_data)
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_topic_moderate_unpin(self):
        ...
        form_data = {}
        response = self.client.post(reverse('foro:topic-unpin', kwargs={'pk': topic.pk, }),
                                    form_data)
        expected_url = topic.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
