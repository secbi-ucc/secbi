...
from foro.views.admin import user, category, comment_flag, config, index, topic
from foro.models.category import Category
from foro.models.comment_flag import CommentFlag, Flag
from foro.forms.admin import UserEditForm, CategoryForm, BasicConfigForm, CommentFlagForm
...
class AdminViewTest(TestCase):
    ...
    def test_user_edit(self):
        ...
        response = self.client.post(reverse('foro:admin-user-edit', kwargs={'user_id': self.user.pk, }),
                                    form_data)
        expected_url = reverse('foro:admin-user-edit', kwargs={'user_id': self.user.pk, })
        self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.get(reverse('foro:admin-user-edit', kwargs={'user_id': self.user.pk, }))
    ...
    def test_user_list(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse('foro:admin-user-list'))
    ...
    def test_user_admins(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse('foro:admin-user-admins'))
        self.assertQuerysetEqual(response.context['users'], map(repr, [self.user, ]))
    ...
    def test_user_mods(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-user-mods'))
    ...
    def test_user_unactive(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-user-unactive'))
        self.assertQuerysetEqual(response.context['users'], map(repr, [unactive, ]))

    def test_index_dashboard(self):
        utils.login(self)
        response = self.client.get(reverse('foro:admin-topic'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_topic_deleted(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-topic-deleted'))
    ...
    def test_topic_closed(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-topic-closed'))
        self.assertQuerysetEqual(response.context['topics'], map(repr, [topic_, ]))
    ...
    def test_topic_pinned(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-topic-pinned'))
        self.assertQuerysetEqual(response.context['topics'], map(repr, [topic_, ]))
    ...
    def test_category_list(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-category-list'))
    ...
    def test_category_create(self):
        ...
        response = self.client.post(reverse('foro:admin-category-create'),
                                    form_data)
        expected_url = reverse("foro:admin-category-list")
        self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.get(reverse('foro:admin-category-create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_category_update(self):
        ...
        utils.login(self)
        form_data = {"parent": "", "title": "foo", "description": "", "is_closed": False, "is_removed": False}
        response = self.client.post(reverse('foro:admin-category-update', kwargs={"category_id": self.category.pk, }),
                                    form_data)
        expected_url = reverse("foro:admin-category-list")
        self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.get(reverse('foro:admin-category-update', kwargs={"category_id": self.category.pk, }))
        self.assertEqual(response.status_code, 200)
    ...
    def test_config_basic(self):
        ...
        response = self.client.post(reverse('foro:admin-config-basic'),
                                    form_data)
        expected_url = reverse('foro:admin-config-basic')
        self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.get(reverse('foro:admin-config-basic'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_flag_open(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-flag-open'))
    ...
    def test_flag_closed(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:admin-flag-closed'))
    ...
    def test_flag_detail(self):
        ...
        response = self.client.post(reverse('foro:admin-flag-detail', kwargs={'pk': comment_flag.pk, }),
                                    form_data)
        expected_url = reverse('foro:admin-flag')
        self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.get(reverse('foro:admin-flag-detail', kwargs={'pk': comment_flag.pk, }))
        self.assertEqual(response.status_code, 200)
