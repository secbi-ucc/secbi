...
from foro.models.topic_private import TopicPrivate
from foro.forms.topic_private import TopicForPrivateForm, TopicPrivateInviteForm,\
    TopicPrivateManyForm, TopicPrivateJoinForm
from foro.templatetags.tags.topic_private import render_invite_form
from foro.views.topic_private import comment_posted
from foro.models.comment import Comment
from foro.signals.topic_private import topic_private_post_create, topic_private_access_pre_create
...
class TopicPrivateViewTest(TestCase):
    ...
    def test_private_publish(self):
        ...
        response = self.client.post(reverse('foro:private-publish'),
                                    form_data)
        private = TopicPrivate.objects.last()
        expected_url = private.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.get(reverse('foro:private-publish'))
    ...
    def test_private_publish_user(self):
        ...
        """
        utils.login(self)
        response = self.client.get(reverse('foro:private-publish', kwargs={'user_id': self.user2.pk, }))
    ...
    def test_private_detail(self):
        ...
        response = self.client.get(reverse('foro:private-detail', kwargs={'topic_id': private.topic.pk,
    ...
    def test_private_access_create(self):
        ...
        response = self.client.post(reverse('foro:private-access-create', kwargs={'topic_id': private.topic.pk, }),
                                    form_data)
        expected_url = private.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_private_access_delete(self):
        ...
        form_data = {}
        response = self.client.post(reverse('foro:private-access-remove', kwargs={'pk': private2.pk, }),
                                    form_data)
        expected_url = private.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_private_access_delete_invalid(self):
        ...
        TopicPrivate.objects.create(user=self.user, topic=private.topic)
        form_data = {}
        response = self.client.post(reverse('foro:private-access-remove', kwargs={'pk': private.pk, }),
                                    form_data)
        self.assertEqual(response.status_code, 404)
    ...
    def test_private_access_delete_leave(self):
        ...
        form_data = {}
        response = self.client.post(reverse('foro:private-access-remove', kwargs={'pk': private2_leave.pk, }),
                                    form_data)
        expected_url = reverse("foro:private-list")
        self.assertRedirects(response, expected_url, status_code=302)
    ...
    def test_private_list(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:private-list'))
    ...
    def test_private_join(self):
        ...
        utils.login(self)
        form_data = {}
        response = self.client.post(reverse('foro:private-join', kwargs={'topic_id': private.topic.pk, }),
                                    form_data)
        expected_url = private.get_absolute_url()
        self.assertRedirects(response, expected_url, status_code=302)

        response = self.client.get(reverse('foro:private-join', kwargs={'topic_id': private.topic.pk, }))
        self.assertEqual(response.status_code, 200)
    ...
    def test_private_join_invalid_regular_topic(self):
        ...
        utils.login(self)
        form_data = {}
        response = self.client.post(reverse('foro:private-join', kwargs={'topic_id': topic.pk, }),
                                    form_data)
        self.assertEqual(response.status_code, 404)
    ...
    def test_private_created_list(self):
        ...
        utils.login(self)
        response = self.client.get(reverse('foro:private-created-list'))
